...
async function filterallinstances()
{
    ...
    instancedata.map((item)=> {
        ...
        item.wfsteps.map( (subitem)=>{
            ...
            if (subitem.sstatus == "pending" && subitem.sscheduledDateTime <= (new Date())) 
        //    if (true) 
           {
            ...
            var messag ='Dear Customer,\n'+ subitem.sName + '\n\nDetails:\nBill ID:' +  item.billId + '\nOpen Bill Amount:' + item.openAmount
            // console.log(messag)
            // axios.post(`http://localhost:9090/api/integration/sendemail`,{esubject:"Debt Bill Notification",ebody:subitem.sName,etoemail:item.emailID}).then( res => {
            axios.post(`http://localhost:9090/api/integration/sendemail`,{esubject:"Debt Bill Notification",ebody:messag,etoemail:item.emailID}).then( res => {
                // console.log(res.data)
                if (res.data = "Ok")
                    {subitemstatus= "success"
                    var data = {'instance_id':item._id, 'firstName' : item.firstName, 'lastName': item.lastName, 'contactNo': item.contactNo, 'emailID' : item.emailID,
                    'sName': subitem.sName, 'sscheduledDateTime': subitem.sscheduledDateTime,  'sstatus': subitemstatus  }}
                else
                    {subitemstatus= "error"
                    var data = {'instance_id':item._id, 'firstName' : item.firstName, 'lastName': item.lastName, 'contactNo': item.contactNo, 'emailID' : item.emailID,
                    'sName': subitem.sName, 'sscheduledDateTime': subitem.sscheduledDateTime,  'sstatus': subitemstatus  }}
                ...
                // block to update the status in instancemodel collection
                 ...
                 axios.patch(`http://localhost:9090/api/instances/updateinstance`, {id:item._id,subid:subitem._id,substatus:subitemstatus})
