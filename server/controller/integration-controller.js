...
fs.readFile(reqDataFilename,async function(err, filedata)
  {
    ...
    jsondata.map((inputrecord) => 
    {
      ...
      if(inputrecord.wfName && inputrecord.firstName && inputrecord.lastName && inputrecord.contactNo && inputrecord.emailID  && inputrecord.billId  && inputrecord.openAmount) 
...
// start to update the data with schedule
      ...
      { 
        ...
        console.log(result)
        callback(result)
        if(result.length<1) console.log("Workflow name in the input file may not be correct!")
      }
      ...
      readSteps(async ()=> {
        ...
        'billId': inputrecord.billId, 'openAmount': inputrecord.openAmount ,'wfId': null, 'wfname':wfName,  'wfsteps': array_iwfsteps, 'instanceStatus' : 'in-progress', 'entrydate': entrydate }
...
try {
  ...
  files.map(async(file)=> 
  { 
    reqDataFilename="C:/Data/Fulda/Summer_Semester2021/1. SDP/server/PaymentFileInput/"+file
     ...
     fs.readFile(reqDataFilename,async function(err, filedata)
   {
    ...
    jsondata.map(async (inputrecord) => 
    {
      ...
      // Updating the openAmount with paymentAmount received for each payment record
       await instancemodel.find({billId:inputrecord.billId}).exec().then(
        async (item) => {
      
          item[0].openAmount = item[0].openAmount - inputrecord.paymentAmount
          var result= await item[0].save()
          // console.log(item[0].openAmount)
          if(item[0].openAmount <= 0 && item[0].instanceStatus === "in-progress")
          {
