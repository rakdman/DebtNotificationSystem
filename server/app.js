...
agenda.define("loadFileJob", async (job) => {
  console.log('Starting the customer and payment file upload batch job as per schedule')
...
async function abc () {
  await agenda.start();
   await agenda.every("2 minutes", "sendEmailJob");
   await agenda.every("2 minutes", "loadFileJob");
}
