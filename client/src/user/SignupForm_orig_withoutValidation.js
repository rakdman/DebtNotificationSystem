...
function SignupForm() {
    ...
    const signupSubmitHandler =  event => {
        ...
        axios.get(`http://localhost:9090/api/user/checkuser`,{params:{username:userName, email:email }}).then(res=>{
        ...
        //    console.log(userName)
           ...
           console.log(response)
            ...
            if ( response == 'UserDonotExist')
            {
                ...
                axios.post(`http://localhost:9090/api/user/createUser`,{username:userName, password:userPassword, email:email,role: "0", firstname:firstName, lastname:lastName })
                .then(res=> console.log(res))

                setErrorMessage(<Typography color="error">User Created Successfully, now wait for Admin to approve</Typography> );
    ...
    return(
        ...
        <center>
            ...
            <center>
                ...
                <TextField label="Email"  variant= "outlined" type="Text" value={email} onChange={event => {setEmail(event.target.value) }}></TextField>
                <br/> 

                <TextField label="User Name"  variant= "outlined" type="Text" value={userName} onChange={event => {setUserName(event.target.value) }}></TextField>
                <br/>  
               
                <TextField label="Password" variant= "outlined" type="Password" value={userPassword} onChange={event=> {setUserPassword(event.target.value)}} ></TextField>
                
                <br/>
                <Button variant="contained" onClick= {signupSubmitHandler} type="submit">Sign Up</Button>
                <br/>
