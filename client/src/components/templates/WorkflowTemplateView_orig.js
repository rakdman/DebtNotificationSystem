...
import { FormControl, Button,Typography,Table,TableRow,TableBody,TableCell} from '@material-ui/core';
  ...
  <Container>
   ...
   <Container width= "40%"> 
          ...
          <Table style = {{ width : "100%"}}>
              <TableBody>
              <TableRow>  
              <TableCell size="medium"> <Typography variant="h7">Workflow ID</Typography> </TableCell>
              <TableCell size="medium"> <Typography variant="h7">Workflow Name</Typography> </TableCell>
              </TableRow>

              {wfData && wfData.map((item,index)=> {

              return ( 
              <TableRow key={item._id} > 
                  <TableCell size="medium"> {item._id} </TableCell>
                  <TableCell size="medium"> {item.wfname} </TableCell>
                  <TableCell size="medium"> {item.wfsteps.map((step)=>{console.log(step.NName)})} </TableCell>
