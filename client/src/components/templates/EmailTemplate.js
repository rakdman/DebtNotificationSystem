...
const saveHandler = event => {
    ...
    axios.post(`http://${process.env.REACT_APP_SERVER_IP_PORT}/api/template/createemail`,{tname:event.templateName,text:event.templateText}).then(res => { console.log(res.data); });
...
return(
    ...
    <React.Fragment>
        ...
        <FormControl onSubmit={submitHandler} size="medium">
            ...
            <TextareaAutosize   rows="5" cols="40" variant="outlined" type="Text"  {...register("templateText",{required: "Input required", maxLength : { value: 160, message: "Maximum 160 characters"} })}/>
            {errors.templateText && errors.templateText.message }
            <br/>
            <ButtonGroup> 
            {/* <Button variant="contained" onClick={saveHandler}>Save</Button> */}
            <Button variant="contained"  size= "small"  color="primary" onClick={handleSubmit(saveHandler)}>Save</Button>
            
            <Button variant="contained"  size= "small"  color="primary" type="submit" onClick={cancelHandler}>Cancel</Button>
            </ButtonGroup>
            <br/>
        </FormControl>
